...
export default function DataTableFilterDemo(props) {
    ...
    const getDateItem = () => {
        ...
        let data = {
            ...
            type: 'gstr-sales-return-report',
        }

        schemaService.getGSTRReports(data).then((response) => {
            console.log(response)
            if (response['data']['status'] === 1) {

                response['data']['data'].map( data =>
                    {
                        data['gross_amt'] = (parseFloat(data['gross_amt'])).toFixed(2)
                        data['tax_amt'] = (parseFloat(data['tax_amt'])).toFixed(2)
                        data['net'] = (parseFloat(data['net'])).toFixed(2)

                        if(data['tax_type'] == 'Local')
                        {
                            data['igst'] = 0
                            data['cgst'] = (parseFloat(data['net'])/2).toFixed(2)
                            data['sgst'] = (parseFloat(data['net'])/2).toFixed(2)
                        } else {
                            data['igst'] = (parseFloat(data['net'])).toFixed(2)
                            data['cgst'] = 0
                            data['sgst'] = 0
                        }
                    })
            ...
            <div className="continerPonitBox">
                ...
                <div className="datatable-filter-demo">
                    <div className="card" style={{ width: '100%', padding: '10px 10px' }}>
                        <h3 style={{ margin: '0px 0px 15px' }}>GSTR Sales Return Report</h3>
                        ...
                        <div style={{ height: '50vh', border: '1px solid #227bce', overflowX: 'scroll' }}>
                            <div style={{ width: '2300px', marginTop: '-5px' }}>
                                <DataTable ref={dt} value={stock}  rowClassName={rowClass} selectionMode="single"
                                    ...
                                    <Column field="billno"      style={{ width: '120px' }} header="Serial" />
                                    <Column field="cname"     style={{ width: 'auto',textAlign:'left' }} header="Customer Name" />
                                    <Column field="tax_amt"     style={{ width: '250px',textAlign:'right' }} header="Customer GSTIN/UIN" />
                                    <Column field="billno"      style={{ width: '120px' }} header="Invoice No" />
                                    <Column field="dat"      style={{ width: '160px' }}   header="Invoice Date" />
                                    <Column field="taxp"     style={{ width: '150px',textAlign:'right' }} header="Rate Of Tax %" />
                                    <Column field="gross_amt"   style={{ width: '150px',textAlign:'right' }} header="Taxable Value" />
                                    <Column field="igst"     style={{ width: '150px',textAlign:'right' }} header="IGST Amount" />
                                    <Column field="cgst"     style={{ width: '150px',textAlign:'right' }} header="CGST Amount" />
                                    <Column field="sgst"     style={{ width: '150px',textAlign:'right' }} header="SGST Amount" />
                                    <Column field="tax_no"       style={{ width: '150px',textAlign:'left' }} header="Tax Amount" />
                                    <Column field="net"        style={{ width: '200px',textAlign:'right' }} header="Total Amount" />
                                </DataTable>
                            </div>
                        </div>
                    </div>
