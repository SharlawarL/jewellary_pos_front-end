        ...
        salesService.getSalesReport(data).then((response) => {
            ...
            if (response['data']['status'] === 1) {
                ...
                response['data']['data'].map((data)=>{
                    data['Cash']    = (Number(data['Cash'])).toFixed(2)
                    data['Card']    = (Number(data['Card'])).toFixed(2)
                    data['Credit']  = (Number(data['Credit'])).toFixed(2)
                    data['Others']  = (Number(data['Others'])).toFixed(2)
                    data['Total']  = (Number(data['Total'])).toFixed(2)
                    total_bills     = parseFloat(total_bills)   + parseFloat(data['billcount'])
                    total_cash      = parseFloat(total_cash)    + parseFloat(data['Cash'])
                    total_card      = parseFloat(total_card)    + parseFloat(data['Card'])
                    total_credit    = parseFloat(total_credit)  + parseFloat(data['Credit'])
                    total_other     = parseFloat(total_other)   + parseFloat(data['Others'])
                    total_all       = parseFloat(total_all)     + parseFloat(data['Total'])
                })

                let blank = { cashier : '.', billcount: '',
                                Cash: '', 
                                Card: '', 
                                Credit : '', 
                                Others : '',
                ...
                let tdata = { cashier : 'Total', billcount: total_bills,
                                Cash: (Number(total_cash)).toFixed(2), 
                                Card: (Number(total_card)).toFixed(2), 
                                Credit : (Number(total_credit)).toFixed(2), 
                                Others : (Number(total_other)).toFixed(2),
                                Total: (Number(total_all)).toFixed(2)}
            ...
            <div className="continerPonitBox">
                ...
                <div className="datatable-filter-demo">
                    <div className="card" style={{ width: '100%', padding: '10px 10px' }}>
                        <h3 style={{ margin: '0px 0px 15px' }}>Daily Estimate Report</h3>
                        ...
                        <div style={{ height: '50vh', border: '1px solid #227bce', overflowX: 'scroll' }}>
                            <div style={{ width: '100%', marginTop: '-5px' }}>
                                <DataTable ref={dt} value={stock} className="p-datatable-striped" globalFilter={globalFilter} emptyMessage="No Search Result." rowClassName={rowClass}>
                                    <Column field="cashier"    style={{ width: '120px' }} header="Cashier" />
                                    <Column field="billcount"  style={{ width: '150px' ,textAlign:'right'}} header="No of Bills" />
                                    <Column field="Cash"       style={{ width: '150px',textAlign:'right' }} header="Cash" />
                                    <Column field="Card"       style={{ width: '150px',textAlign:'right' }} header="Card" />
                                    <Column field="Credit"     style={{ width: '150px' ,textAlign:'right'}} header="Credit" />
                                    <Column field="Others"     style={{ width: '200px',textAlign:'right' }} header="Others" />
                                    <Column field="Total"     style={{ width: '200px',textAlign:'right' }} header="Total"    style={{ width: '200px',textAlign:'right' }} header="Total" />
                                </DataTable>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-grid" style={{ padding: '10px 15px' }}>
                    ...
                    <div className="p-col-1 submitFormButton">
                        <Link to="/estimate-daily-report" className="Link">
