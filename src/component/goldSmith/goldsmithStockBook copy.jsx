...
export default function DataTableFilterDemo(props) {
    ...
    const [opnBc, setOpnBc]     = useState(null);
    ...
    const getItem = () => {
        ...
        goldsmithService.getSmithDayBook(data).then((response) =>{
            console.log(response)
            if (response['data']['status'] === 1) {
                let temp = 0;
                response['data']['data']['daybook'].map((data) =>{
                    temp = Number(temp) + Number(data['credit'])
                    data['closing'] = temp
                })

                setStock(response['data']['data']['daybook']);
                setOpnBc(response['data']['data']['open_blc'])
                setDat(response['data']['data']['dat'])
            ...
            <div className="continerPonitBox">
                ...
                <div className="datatable-filter-demo">
                    <div className="card" style={{ width: '100%', padding: '10px 10px' }}>
                        <h3 style={{ margin: '0px 0px 15px' }}>GOLD Smith Day Book</h3>
                        <div className="p-grid-data">
                            <div style={{ padding: '10px 10px', width:'400px', textAlign: "left" }}>
                                Opening Balance on {dat}  : {opnBc}
                            </div>
                        ...
                        <div style={{ height: '50vh', border: '1px solid #227bce', marginTop:'10px', overflowX: 'scroll' }}>
                            <div style={{ width: '100%', marginTop: '-5px' }}>
                                <DataTable ref={dt} value={stock} className="p-datatable-striped" globalFilter={globalFilter} emptyMessage="No Search Result." rowClassName={rowClass}>
                                    <Column style={{textAlign:'center',width: '80px'}} field="sno"  header="Sr No" />
                                    <Column style={{textAlign:'center',width: '100px'}} field="billno"  header="Bill No" />
                                    <Column field="gname"  header="GOLD Smith Name" />
                                    <Column style={{textAlign:'right'}} field="debit"  header="Debit" />
                                    <Column style={{textAlign:'right'}} field="credit"  header="Credit" />
                                    <Column style={{textAlign:'right'}} field="closing"  header="Closing" />
                                </DataTable>
                            </div>
                        </div>
                    </div>
