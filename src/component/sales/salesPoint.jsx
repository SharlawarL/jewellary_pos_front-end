...
export default class salesPointComponent extends React.Component {
    ...
    constructor(props) {
        ...
        // this.onChangeReceived = this.onChangeReceived.bind(this)
    ...
    componentDidUpdate() {
     ...
     //   console.log(this.state.selectedItem)
        ...
        const gross = parseFloat(this.state.weight)?parseFloat(this.state.weight):0;
        const wastage = parseFloat(this.state.westage)?parseFloat(this.state.westage):0;
        const waste = (gross * wastage) / 100;
        net_total = gross + waste



        // this.state.totalQty = totalQty?totalQty:0
        // this.state.totalGrossWeight = totalWeight?totalWeight.toFixed(3):0
        // this.state.totalRate = totalRate?totalRate.toFixed(2):0
        // this.state.total_making = totalMaking?(parseFloat(totalMaking)).toFixed(2):0
        // this.state.totalItems = totalStocks?totalStocks:0
        // alert(totalAmt)
        // this.state.tax_amt = 
        total_sub = parseFloat(totalAmt)?(parseFloat(totalAmt)).toFixed(2):0
        des_amt = (parseFloat(this.state.des_per) * total_sub)?(parseFloat((parseFloat(this.state.des_per) * total_sub) / 100)).toFixed(2):0
        afterDes = parseFloat(totalRate)?(parseFloat(totalRate)).toFixed(2):0
        ...
        grand_total = ((parseFloat(total_sub)  + parseFloat(totalMaking)) - parseFloat(amountOldTotal) - parseFloat(des_amt)).toFixed(2)

        tax_amt = (parseFloat((this.state.tax_per * parseFloat(grand_total)) / 100)).toFixed(2);
        
        // grand_total = (parseFloat(grand_total) + parseFloat(totalMaking)) - parseFloat(amountOldTotal) 

        if (this.state.gross_weight && this.state.loss_weight) {
            net_total = ( parseFloat(this.state.gross_weight) - parseFloat(this.state.loss_weight))
        } else {
            net_total = parseFloat(grand_total) + parseFloat(tax_amt) + parseFloat(this.state.others_add?this.state.others_add:0) + parseFloat(this.state.others?this.state.others:0)
        }
        ...
        setTimeout(() => {
            this.setState({
                totalQty: totalQty,
                totalGrossWeight: totalWeight?totalWeight.toFixed(3):0,
                totalRate: totalRate?totalRate.toFixed(2):0,
                total_making: totalMaking?(parseFloat(totalMaking)).toFixed(2):0,
                totalItems: totalStocks?totalStocks:0,
                des_amt : (parseFloat(this.state.des_per) * total_sub)?(parseFloat((parseFloat(this.state.des_per) * total_sub) / 100)).toFixed(2):0,
                total_sub : total_sub,
                afterDes : parseFloat(totalRate)?(parseFloat(totalRate)).toFixed(2):0,
                qtyOldTotal: qtyOldTotal,
                weightOldTotal: weightOldTotal,
                amountOldTotal: amountOldTotal,
                tax_amt: tax_amt,
                grand_total: grand_total,
                net_total: net_total,
                blc_total: (parseFloat(net_total - this.state.tender_total)).toFixed(2)
            })
        }, 100)
      }
    ...
    //     this.setState({
    //         [e.target.name]: e.target.value,
    //         tax_amt: (parseFloat((e.target.value * parseFloat(this.state.totalRate - this.state.des_amt)) / 100)).toFixed(2)
    //     })
    // }
    ...
    onChangeFilter(e){
        // var data = this.state.stock.filter(e.target.value)
        var type = '';
        this.state.selectedItem.map((data)=>
        {
        ...
        for (let data of this.state.stock) {
         //   console.log(count)
            if (data.lot_no === e.target.value) {
                ...
                if(data.item_type  == 'Silver')
                {
                    price = localStorage.getItem("Sil_rate");
                }
                this.setState({
        ...
        if ((event.keyCode === 9)) {
         ...
         //   console.log("Lots number --->>>",this.state.lot_no)
            if (this.state.lot_no === '' ) {
                ...
                {
                    if(this.state.selecteddPay == 'Multipay')
                        this.openModelTender()
                    if(this.state.selecteddPay == 'Card')
                    {
                        this.setState({
                            tender_total: (parseFloat(this.state.net_total?this.state.net_total:0)).toFixed(2)
                        }) 
                    } else {
                        this.setState({
                            tender_total: ''
                    ...
                    form.elements[39 + 2*this.state.selectedItem.length].focus();
                    event.preventDefault();
                ...
                } else if(index == (39 + 2*this.state.selectedItem.length)) 
                {
                 //   console.log("index 33", (35+ 2*this.state.selectedItem.length))
                    form.elements[8 + 2*this.state.selectedItem.length].focus();
                    event.preventDefault();
    ...
    getTodaysList(){
        ...
        companyService.getTodaysValue(data).then((response) => {
            if (response['data']['status'] === 1) {
                this.setState({
                    todaysDefaults: response['data']['data'],
                })
            }
        }).catch((error) => {
         //   console.log(error)
        ...
        customerService.getCustomer(data).then((response) => {
            ...
            if (response['data']['status'] === 1) {
                for (let data of response['data']['data']) {
                    let temp = { name: data['cid'], code: data['cid'], cname: data['cname'], mobile: data['mobile'], cid: data['cid'], add1: data['add1'], category: data['category'], tax_no: data['tax_no'] }
                    let tempMobile = { name: data['mobile'], code: data['mobile'], cname: data['cname'], mobile: data['mobile'], cid: data['cid'], add1: data['add1'], category: data['category'], tax_no: data['tax_no'] }
    ...
    submitStock(e) {
        ...
        if (Number(this.state.qty) <= Number(this.state.qty_total)) {
            if ((this.state.stack != 0) && (this.state.item_no != 0)) {
                ...
                var net_weight_temp = (parseFloat(westage) == 0)?parseFloat(weight):parseFloat(weight) + parseFloat((parseFloat(weight)*parseFloat(westage))/100)
                // alert(price_type)
                ...
                if (price_type === 'Weight Wise Rate') {
                    net_weight_data = parseFloat(net_weight_temp) * parseFloat(qty);
                    amount_data = parseFloat(rate) * parseFloat(net_weight_temp);
                }
                if (price_type === 'Piece Wise Rate') {
                    amount_data = parseFloat(price) * parseFloat(qty);
                    net_weight_data = net_weight_temp;
                ...
                this.state.selectedItem.map( data =>{
                    if((data['lot_no']  == lot_no) && data['name']  == name)
                    {
                        data['rate'] =  parseFloat(data['rate']) + parseFloat(rate)
                        data['amount'] =  parseFloat(data['amount']) + parseFloat(amount_data)
                        data['weight'] =  parseFloat(data['weight']) + parseFloat(weight)
                        data['net_weight'] =  parseFloat(data['net_weight']) + parseFloat(net_weight_temp)
                        data['westage'] =  parseFloat(data['westage']) + parseFloat(westage)
                        data['making_charges'] =  parseFloat(data['making_charges']) + parseFloat(making_charges)
                        data['total'] =  (parseFloat(parseFloat(data['total']) + parseFloat(totalAmt))).toFixed(2)
                        data['qty'] =  parseFloat(data['qty']) + parseFloat(qty)
                        status = false
                        this.setState({
                            lot_no: '',
                            stack: '',
                ...
                if(status)
                {
                    let data = {
                        ...
                        net_weight: net_weight_data ? parseFloat(net_weight_data) :'',
                        westage: westage ? parseFloat(westage) :'',
                        making_charges: making_charges ? parseFloat(making_charges) :'',
                        total: totalAmt ? parseFloat(totalAmt) :'',
    ...
    submitOldIem(e) {
        ...
        if (this.state.qtyOld != 0) {
            ...
            let qtyOldTotal = 0
            let weightOldTotal= 0
            let amountOldTotal= 0
            ...
            let data = {
                nameOld: nameOld ? nameOld :'',
                type: type ? type.name : '',
                purity: purity ? purity : '',
                qtyOld: qtyOld ? qtyOld :'',
                weightOld: weightOld ? weightOld :'',
                rateOld: rateOld ? rateOld :'',
                amountOld: weightOld ? weightOld*rateOld :'',
    ...
    submitStackForm = (e) => {
        ...
        let data = {
            ...
            descount: des_per?Number(des_per):0,
            desamt: des_amt?des_amt:0,
            afterDes: afterDes?afterDes:0,
            selectedItem: JSON.stringify(selectedItem),
            selectedOldItem : JSON.stringify(selectedOldItem),
            totalItems: totalItems,
            making_charges: total_making,
            tax_amt: tax_amt,
            tax_per: tax_per?Number(tax_per):0,
            bal: blc_total,
            ...
            othersAdd:others? parseFloat(others):'.',
            ctype: ctype?ctype:'B2C',
            ctaxno: ctaxno?ctaxno:'.',
            
            aname: aname?aname:'.',
            acode : agentCode?agentCode:'.',

            address: address?address:'.',
            remark: remark?remark:'.',
            reference: reference?reference:'.',

            cash : tender_cash?tender_cash:'.',
            card : tender_card?tender_card:'.',
            others : tender_other?tender_other:'.',
            pay_remarks : pay_remarks?pay_remarks:'.'
        }
        ...
        if (this.state.selectedItem.length > 0) {
            ...
            if((this.state.userlevel == "Administrator") || (data.type == 'new'))
            {
                ...
                if (response['data']['status'] === 1) {
                    ...
                    this.clearForm();
                    this.setState({
                        printBillNo: response['data']['data'],
                        success: true,
                        printModelBox: true,
                    });
                } else {
    ...
    getLastBill = () => {
        ...
        salesService.getViewBill(data).then((response) => {
            if (response['data']['status'] === 1) {
                ...
                response['data']['data']['selectedOldItem'].map(data => {
                    data['nameOld'] = data['iname']
                    data['type'] = data['itype']
                    data['purity'] = data['qual']
                    data['qtyOld'] = data['quan']
                    data['weightOld'] = data['weight']
                    data['rateOld'] = data['rate']
                ...
                this.setState({
                    ...
                    selectedOldItem: response['data']['data']['selectedOldItem'],
                    customer_id: response['data']['data'][0]['cid'],
                    mobile: response['data']['data'][0]['mobile'],
                    customer_name: response['data']['data'][0]['cname'],
                    agentCode: response['data']['data'][0]['acode'],
                    aname: response['data']['data'][0]['aname'],
                    remark: response['data']['data'][0]['remarks'],
                    reference : response['data']['data'][0]['reference'],
                    net_total:  parseFloat(response['data']['data'][0]['net']),
                    others:  parseFloat(response['data']['data'][0]['add_amt']),
                    des_per: parseFloat(response['data']['data'][0]['dis_per']),
                    des_amt: parseFloat(response['data']['data'][0]['dis_amt']),

                    // amountOldTotal: parseFloat(response['data']['data'][0]['old_amt']),

                    tender_cash: parseFloat(response['data']['data'][0]['cash']),
                    tender_card: parseFloat(response['data']['data'][0]['card']),
                    tender_other: parseFloat(response['data']['data'][0]['others']),

                    pay_remarks: response['data']['data'][0]['pay_remarks'],
         ...
         //   console.log(error)
            this.toast.show({severity:'error', summary: '!', detail: 'Check Connection', life: 3000});
        })
    }
        ...
        salesService.getViewBill(data).then((response) => {
            if (response['data']['status'] === 1) {
                ...
                this.setState({
                    ...
                    net_total:  parseFloat(response['data']['data'][0]['net']),
                    others:  parseFloat(response['data']['data'][0]['add_amt']),
                    des_amt:  parseFloat(response['data']['data'][0]['tax_amt']),
                    des_per: parseFloat(response['data']['data'][0]['dis_per']),
                ...
                // toast.error(response['data']['message']);
                if(this.state.billno < this.state.newbillno)
                {
                    this.setState({
                        billno: this.state.billno + 1,
                        loggedIn: false,
    ...
    clearFormNext = () => {
        this.setState(
            {
                lot_no:'',
                billno: ((this.state.billno > 0) && (this.state.billno < this.state.newbillno))?this.state.billno:this.state.newbillno,
                selectedItem: [],
                item_code: '',
                rate: '',
                weight: '',
    ...
    clickCustomerTable = (event) => {
     //   console.log(event)
        this.setState({
            ...
            ctype: event.value.category,
            ctaxno: event.value.tax_no,
        ...
        salesService.getRetriveBill(data).then((response) => {
            if (response['data']['status'] === 1) {
                ...
                this.setState({
                    ...
                    customer_name: response['data']['data'][0]['cname'],
                    agentCode: response['data']['data'][0]['acode'],
                    reference: response['data']['data'][0]['aname'],
                    remark: response['data']['data'][0]['remarks'],
                    viewModelBox: false,
                    retrivalModexBox: false
                })
    ...
    clickItemTable = (event) => {
     //   console.log(event.value)
        ...
        //     price = localStorage.getItem("Gold_rate");
        // }
        // if(event.value.item_type  == 'Silver')
        // {
        //     price = localStorage.getItem("Sil_rate");
        // }
        if(event.value.price_type == 'Piece Wise Rate')
        {
            price = event.value.price
        } else {
            this.state.todaysDefaults.map((data) =>
            {
                if((data.item_type == event.value.item_type) && (data.purity == event.value.purity))
                {
                    price = data.price
                } 
            })
     ...
     //   console.log(event)
        ...
        if(this.state.modelBox)
        {
            ...
            this.setState({
                ...
                rate        : price,
                weight      : event.value.net_weight?event.value.net_weight:0,
                net_weight  : parseFloat(event.value.net_weight) + parseFloat(event.value.net_weight/event.value.wastage),
                westage     : event.value.wastage?event.value.wastage:'',
                making_charges: event.value.making_charge?event.value.making_charge:0,
                qty         : 1,
            ...
            <div className="body">
                            ...
                            <div style={{ width: '76%', borderRight: '1px solid silver', padding: '0px 20px', overflow: 'hidden' }}>
                                ...
                                <div className="p-grid pos-header" style={{ color: 'white', textShadow: '0px 0px 5px black', marginTop: '-20px', marginLeft: '-20px', width: '105%', border: '1px solid silver' }}>
                                    <div className="p-col-4" >
                                        <h3 style={{ margin: '0px', color:'#4f3f3f', textShadow:'none' }}>
                                            Point of Sale
                                        </h3>
                                    </div>
                                        ...
                                        <div className="p-grid">
                                            ...
                                            <div className="p-col-2 column-input">
                                                <div className="p-field">
                                                    ...
                                                    <InputText id="username1" value={((this.state.westage!=0)?parseFloat(this.state.weight) + parseFloat((parseFloat(this.state.weight*this.state.westage))/100):parseFloat(this.state.weight)).toFixed(2)} name="net_weight" aria-describedby="username1-help" onChange={this.onChange} onClick={this.onChangeEmpty} onKeyDown={this.handleEnter} className="p-d-block InputPrimeBox" />
                                                </div>
                                            </div>
                                            <div className="p-col-2 column-input">
                                                <div className="p-field">
                                ...
                                <div className='table-style' id="pos-tab">
                                    <div style={{ width: '2000px', marginTop: '-5px' }}>
                                        ...
                                        <DataTable state={{ overflowY: 'scroll' }} editMode="row" onRowEditInit={this.onRowEditInit} onRowEditCancel={this.onRowEditCancel}
                                            value={this.state.selectedItem} first={this.state.first} onPage={(e) => this.setState({ first: e.first })}
                                            >
                                            <Column body={this.trashSelectedItem} headerStyle={{ width: '30px', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} />
                                            <Column style={{ width: '70px' }}   field="sr_no" header="Sno" editor={(props) => this.codeEditor('selectedItem', props,'sr_no')}/>
                                            <Column style={{ width: '70px' }}   field="lot_no" header="Lot_No" editor={(props) => this.codeEditor('selectedItem', props,'lot_no')}/>
                                            <Column style={{ width: '180px' }}  field="name" header="Name"/>
                                            <Column style={{ width: '120px' }}  field="qty" header="Qty/Pcs"/>
                                            <Column style={{ width: '100px',textAlign:'right' }}  field="weight" header="Weight"/>
                                            <Column style={{ width: '120px',textAlign:'right' }} field="westage" header="Wastage %"/>
                                            <Column style={{ width: '120px',textAlign:'right' }} field="net_weight" header="Total Weight"/>
                                            <Column style={{ width: '120px',textAlign:'right' }} field="rate" header="Rate"/>
                                            <Column style={{ width: '120px',textAlign:'right' }} field="amount" header="Amount"/>
                                            <Column style={{ width: '150px',textAlign:'right' }} field="making_charges" header="Making Charges"/>
                                            <Column style={{ width: '120px',textAlign:'right' }} field="total" header="Total"/>
                                            <Column style={{ width: '100px' }} field="hsn_code" header="HSN"/>
                                            <Column style={{ width: '120px' }} field="item_code" header="Item Code"/>
                                ...
                                <div style={{ marginTop: '10px', display: 'flex' }}>
                                    <div className="column-input" style={{width:'8%'}}>
                                ...
                                <InputText id="username1" aria-describedby="username1-help" value={this.state.customer_id} name="customer_id" onChange ={this.onChange} onKeyDown={this.handleEnter} className="p-d-block InputPrimeBox"/>
                                    ...
                                    <div className="column-input" style={{width:'14%'}}>
                                        <div className="p-field">
                                    ...
                                    <div className="column-input" style={{width:'20%'}}>
                                        <div className="p-field">
                                    ...
                                    <div className="column-input" style={{width:'10%'}}>
                                        <div className="p-field">
                                            <label htmlFor="username1" className="p-d-block" style={lebelStyle}>Client Code</label>
                                    ...
                                    <div className="column-input" style={{width:'20%'}}>
                                        <div className="p-field">
                                            <label htmlFor="username1" className="p-d-block" style={lebelStyle}>Client Name</label>
                                    ...
                                    <div className="column-input" style={{width:'35%'}}>
                                        <div className="p-field">
                                ...
                                <div className="p-grid" style={{marginLeft:'0px',marginRight:'0px'}}>
                                    ...
                                    <div className="p-col-2 submitFormButton">
                                        <Button className="inputData buttonSecondary" variant="contained" onClick={this.getLastBill} onKeyDown={this.handleEnter}>
                                            <div className="buttonText"> <i className="pi pi-angle-double-left"></i> <span className="buttonTextFirstLetter">L</span>ast <span className="buttonTextFirstLetter">B</span>ill</div>
                                        </Button>
                                    </div>
                                    <div className="p-col-2 submitFormButton">
                                        <Button className="inputData buttonSecondary" variant="contained" onClick={this.getNextBill} onKeyDown={this.handleEnter}>
                                            <div className="buttonText"><span className="buttonTextFirstLetter">N</span>ext <span className="buttonTextFirstLetter">B</span>ill  <i className="pi pi-angle-double-right"></i> </div>
                                        </Button>
                                    </div>
                                    <div className="p-col-2 submitFormButton">
                                    ...
                                    <div className="p-col-2 submitFormButton">
                                        ...
                                        {(this.state.newbillno === this.state.billno) ?
                                            ...
                                            <Button onClick={this.openModelDelete} className="inputData buttonSecondary" variant="contained" onKeyDown={this.handleEnter}>
                                ...
                                <div className="p-grid">
                                    ...
                                    <div className="p-col-12 right_total_box" style={{height:'40px'}}>
                                        <div className="p-grid">
                                            ...
                                            <div className="p-col-8">
                                                <div className="p-grid">
                                                    <InputText className="p-col-6 right_total_box_amount" aria-describedby="username1-help" name="todaydate" value={this.state.todaydate} disabled />
                                                    <InputText className="p-col-6 right_total_box_amount" aria-describedby="username1-help" name="currentTime" value={this.state.currentTime}  disabled />
                                                </div>
                                            </div>
                                            {/* <div className="p-col-8 right_total_box_amount">
                                    ...
                                    <div className="p-col-12 right_total_box" style={{height:'40px'}}>
                                        <div className="p-grid">
                                            ...
                                            <div className="p-col-8">
                                                <div className="p-grid">
                                                    <InputText className="p-col-6 right_total_box_amount" aria-describedby="username1-help" value={this.state.totalItems} onKeyDown={this.handleEnter} disabled />
                                                    <InputText className="p-col-6 right_total_box_amount" aria-describedby="username1-help" value={this.state.totalQty} onKeyDown={this.handleEnter}  disabled />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-col-12 right_total_box">
                                    ...
                                    <div className="p-col-12 right_total_box">
                                        <div className="p-grid">
                                            ...
                                            <InputText className="p-col-8 right_total_box_amount" aria-describedby="username1-help"  value={this.state.total_sub} disabled/>
                                            {/* <div className="p-col-8 right_total_box_amount">
                                    ...
                                    <div className="p-col-12 right_total_box">
                                        <div className="p-grid">
                                            ...
                                            <InputText className="p-col-8 right_total_box_amount" aria-describedby="username1-help" name="totalRate" value={parseFloat(this.state.grand_total)} onKeyDown={this.handleEnter} onChange={this.onChange} />
                                            {/* <div className="p-col-8 right_total_box_amount">
                                            0.00
                                    </div> */}
                                        </div>
                                        ...
                                        <div className="p-grid">
                                            ...
                                            <div className="p-col-8">
                                                ...
                                                <div className="p-grid">
                                                    ...
                                                    <InputText className="p-col-6 right_total_box_amount" aria-describedby="username1-help" name="tax_amt" value={this.state.tax_amt} onKeyDown={this.handleEnter} onChange={this.onChange} />
                                                </div>
                                            </div>
                                    ...
                                    <div className="p-col-12 right_total_box">
                                        <div className="p-grid">
                                            ...
                                            <InputText className="p-col-8 right_total_box_amount" aria-describedby="username1-help" id="others_add" name="others_add" value={this.state.others_add} onKeyDown={this.handleEnter} onClick={this.onChangeEmpty} onChange={this.onChange} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-col-12" style={{ padding: '0px' }}>
                                        <div className="p-grid right_final_total_box">
                                            ...
                                            <div className="p-col-8" style={{textAlign: 'right', marginTop:'-11px' }}>
                                                <input style={{fontSize:'1.83em',width:'106%',background: 'none',outline:'none', border: 'none',textAlign: 'right'}} className="p-col-8" aria-describedby="username1-help" name="net_total" value={(parseFloat(this.state.net_total)).toFixed(2)} onKeyDown={this.handleEnter} onChange={this.onChange} />
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-col-12" style={{ padding: '0px' }}>
                                    ...
                                    <div className="p-col-12" style={{ padding: '0px' }}>
                                        <div className="p-grid right_final_total_box">
                                            ...
                                            <div className="p-col-8" style={{ textAlign: 'right', marginTop:'-11px'  }}>
                                                <input style={{fontSize:'1.83em',width:'106%',background: 'none',outline:'none', border: 'none',textAlign: 'right'}} className="p-col-8" aria-describedby="username1-help" name="net_total" value={(parseFloat(this.state.blc_total?this.state.blc_total:0)).toFixed(2)} onKeyDown={this.handleEnter} onChange={this.onChange} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <Dialog header="Old Items Entry" visible={this.state.modelBox} modal style={{ width: '80vw', height: 'auto', backgroundColor: 'white', padding: '10px 0px 0px' }} onHide={this.openModel}
                    ...
                    <div>
                        <form onSubmit={this.submitOldIem} autoComplete="off">
                            <div className="p-grid" style={{ margin: '0px',padding: '20px 10px' }}>
                                <div className="p-col-2 column-input-old">
                                    <span className="p-field">
                                        ...
                                        <input list="browsers" id="old_item_name" placeholder="Enter Item Name" style={{width:'100%',border:'1px solid silver',borderRadius:'3px',padding:'10px 10px'}} name="nameOld" value={this.state.nameOld}  onChange={this.onChangeFilter} />
                                ...
                                <div className="p-col-2 column-input-old">
                                    <span className="p-field">
                                        ...
                                        <Dropdown name="type" value={this.state.type} options={this.state.typeList} onChange={this.handleselectedType} onKeyDown={this.handleEnterOld} style={{ width: '100%', padding: '0px'}} optionLabel="name" />
                                    </span>
                                </div>
                                <div className="p-col-2 column-input-old">
                                    <span className="p-field">
                                        ...
                                        <input list="purity" placeholder="Enter Purity" style={{width:'100%',border:'1px solid silver',borderRadius:'3px',padding:'10px 10px'}} id="browser" name="purity" value={this.state.purity}  onChange={this.onChangeFilter} />
                                        <datalist id="purity" >
                                            {  
                                                this.state.purityList.map((item,index)=>{
                                                    return  <option key={index} value={item.name} />;
                                                })
                                            }   
                                        </datalist>
                                ...
                                <div className="p-col-2 column-input-old">
                                    <span className="p-field">
                                        ...
                                        <InputText id="billno" aria-describedby="username1-help" name="amountOld" value={this.state.weightOld*this.state.rateOld} onChange={this.onChange} onKeyDown={this.handleEnterOld} className="p-inputtext-sm p-d-block p-mb-2 InputPrimeBox" />
                        ...
                        <div  style={{ height: '250px', border: '2px solid #b0a939', overflowX: 'scroll' }}>
                            <div style={{ width: '1000px', marginTop: '-5px' }}>
                                <DataTable state={{ overflowY: 'scroll' }} style={{width:'1200px'}}
                                    ...
                                    <Column style={{ width: '250px' }} field="nameOld" header="Item Name"/>
                                    <Column style={{ width: '150px' }} field="type" header="Type"/>
                                    <Column style={{ width: '200px' }} field="purity" header="Purity"/>
                                    <Column style={{ width: '150px' }} field="qtyOld" header="PCS Qty"/>
                                    <Column style={{ width: '150px' }} field="weightOld" header="Weight"/>
                                    <Column style={{ width: '150px' }} field="rateOld" header="Rate / grm"/>
                                    <Column style={{ width: '150px' }} field="amountOld" header="Amount"/>
                        ...
                        <div className="p-grid" style={{ margin: '0px 0px' }}>
                            ...
                            <div className="p-col-2">
                                <Button type="submit" className="inputData buttonSecondary" variant="contained" onClick={this.openModelClose}>
                                    <div className="buttonText"> <i className="pi pi-times-circle"></i> <span className="buttonTextFirstLetter">C</span>lear</div>
                                </Button>
                            </div>  
                        </div>
                    </div>
                ...
                <Dialog header="Tender" visible={this.state.tenderModelBox} modal style={{ width: '40vw', height: 'auto', backgroundColor: 'white' }} onHide={this.openModelTender}
                    ...
                    <form>
                        <div className="p-grid" style={{ margin: '20px' }}>
                            ...
                            <div className="p-col-9 column-input">
                                <div className="p-field">
                                    <InputText id="username1" aria-describedby="username1-help" name="ten_total" value={(parseFloat(this.state.net_total?Number(this.state.net_total):0) + parseFloat(this.state.others_add?this.state.others_add:0) - parseFloat(this.state.others?this.state.others:0)).toFixed(2)} onChange={this.onChangeTender} onKeyDown={this.handleEnterTender} className="p-d-block InputPrimeBox" />
                                </div>
                            </div>
                            <div className="p-col-3 column-input">
                            ...
                            <div className="p-col-9 column-input">
                                <div className="p-field">
                                    <InputText id="username1" aria-describedby="username1-help" name="tender_total" value={(parseFloat(this.state.tender_cash?this.state.tender_cash:0) + parseFloat(this.state.tender_card?this.state.tender_card:0) + parseFloat(this.state.tender_other?this.state.tender_other:0)).toFixed(2)} onChange={this.onChangeTender} onKeyDown={this.handleEnterTender} className="p-d-block InputPrimeBox" />
                                </div>
                            </div>
                            <div className="p-col-3 column-input">
                            ...
                            <div className="p-col-9 column-input">
                                <div className="p-field">
                                    <InputText id="username1" aria-describedby="username1-help" name="ten_total" value={(parseFloat(this.state.tender_total) - (parseFloat(this.state.tender_cash?this.state.tender_cash:0) + parseFloat(this.state.tender_card?this.state.tender_card:0) + parseFloat(this.state.tender_other?this.state.tender_other:0)).toFixed(2)).toFixed(2)} onChange={this.onChangeTender} onKeyDown={this.handleEnterTender} className="p-d-block InputPrimeBox" />
                                </div>
                            </div>
                    ...
                    <div style={{ height: '350px', border: '2px solid #b0a939', overflowX: 'scroll' }}>
                        <div style={{ width: 'auto', marginTop: '-5px' }}>
                            ...
                            <DataTable autofocus id="item_table" state={{ overflowY: 'scroll' }} selectionMode="single" selection={this.state.selectedProducts} dataKey="item_no"
                                ...
                                <Column style={{ width: '70px' }} field="lot_no" header="Lot_no"/>
                                <Column style={{ width: '120px' }} field="item_no" header="Item Code"/>
                                <Column style={{ width: '180px' }} field="name" header="Name"/>
                                <Column style={{ width: '120px' }} field="quan" header="Qty/Pcs"/>
                                <Column style={{ width: '100px' }} field="hsn_code" header="HSN"/>
                                <Column style={{ width: '200px' }} field="price_type" header="Price Type"/>
                            </DataTable>
                            {/* </ScrollBox> */}
                    ...
                    <div style={{ height: '300px', border: '2px solid #b0a939', overflowX: 'scroll'}}>
                        <div style={{ width: 'auto', marginTop: '-5px' }}>
                            ...
                            <DataTable state={{ overflowY: 'scroll' }} selectionMode="single" dataKey="cid" globalFilter={ this.state.mobile || this.state.customer_name || this.state.customer_id}
                                ...
                                <Column style={{ width: '100px' }} field="cid" header="Customer ID" />
                                <Column style={{ width: '120px' }} field="cname" header="Customer Name"/>
                                <Column style={{ width: '200px' }} field="add1" header="Address"/>
                                <Column style={{ width: '100px' }} field="mobile" header="Mobile"/>
                                <Column style={{ width: '200px' }} field="email" header="Email"/>
                            </DataTable>
                            {/* </ScrollBox> */}
                        </div>
                    </div>
                    ...
                    <div style={{ height: '300px', border: '2px solid #b0a939', overflowX: 'scroll' }}>
                        <div style={{ width: 'auto', marginTop: '-5px' }}>
                            ...
                            <DataTable state={{ overflowY: 'scroll' }} selectionMode="single" dataKey="cid" globalFilter={this.state.reference}
                                ...
                                <Column style={{ width: '100px' }} field="acode" header="Agent Code" />
                                <Column style={{ width: '120px' }} field="aname" header="Agent Name"/>
                                <Column style={{ width: '200px' }} field="add1" header="Address"/>
                                <Column style={{ width: '100px' }} field="mobile" header="Mobile"/>
                                <Column style={{ width: '200px' }} field="email" header="Email"/>
                            </DataTable>
                    ...
                    <div style={{ height: '400px', border: '2px solid #b0a939', overflowX: 'scroll', margin:'30px 10px 10px' }}>
                        <div style={{ width: 'auto', marginTop: '-5px' }}>
                            ...
                            <DataTable state={{ overflowY: 'scroll' }} selectionMode="single" dataKey="cid" globalFilter={ this.state.mobile || this.state.customer_name || this.state.customer_id}
                                ...
                                <Column style={{ width: '100px' }} field="billno" header="Bill Number" />
                                <Column style={{ width: '120px' }} field="paid" header="Bill Amount" />
                                <Column style={{ width: '200px' }} field="quans" header="Total Items" />
                                <Column style={{ width: '100px' }} field="cashier" header="Cashier" />
                                <Column style={{ width: '200px' }} field="system_ip" header="System Ip"/>
                            </DataTable>
                            {/* </ScrollBox> */}
                        </div>
                    </div>
                ...
                <Dialog header="View Bill" visible={this.state.permissionModelBox} modal style={{ width: '30vw', height: 'auto', backgroundColor: 'white', padding:'20px'}} onHide={this.openModelPermission}
                    ...
                    <div className="p-field">
                        ...
                        <label htmlFor="username1" className="p-d-block"  style={lebelStyle,{fontWeight:'400'}}>Contact Administratore for more details.</label>
                    </div>
                    <div className="row">
                        <div className="col-6">
