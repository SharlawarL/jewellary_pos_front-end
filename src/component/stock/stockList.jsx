        ...
        stockService.getStock(data).then((response) =>{
            ...
            if (response['data']['status'] === 1) {
                ...
                response['data']['data'].map((data)=>{
                    data['gross_weight']  = (parseFloat(data['gross_weight'])).toFixed(3)
                    data['less_weight']  = (parseFloat(data['less_weight'])).toFixed(3)
                    data['net_weight']  = (parseFloat(data['net_weight'])).toFixed(3)
                    data['total_weight']  = (parseFloat(data['total_weight'])).toFixed(3)
                    data['net_value']  = (parseFloat(data['net_value'])).toFixed(2)
                    data['price']  = (parseFloat(data['price'])).toFixed(2)
                    total_stock       = parseFloat(total_stock)     + parseFloat(data['quan'])
                    gross_weight       = parseFloat(gross_weight)     + parseFloat(data['gross_weight'])
                    less_weight       = parseFloat(less_weight)     + parseFloat(data['less_weight'])
                    net_weight       = parseFloat(net_weight)     + parseFloat(data['net_weight'])
                    total_weight       = parseFloat(total_weight)     + parseFloat(data['total_weight'])
                })
                ...
                let tdata = { lot_no:'Total',
                                quan : total_stock,
                                gross_weight : (parseFloat(gross_weight)).toFixed(3),
                                less_weight : (parseFloat(less_weight)).toFixed(3),
            ...
            <div className="continerPonitBox">
                ...
                <div className="datatable-filter-demo">
                    <div className="card" style={{ width: '100%', padding: '10px 10px' }}>
                        <h3 style={{ margin: '0px 0px 15px' }}>Stock List</h3>
                        ...
                        <div style={{ height: '50vh', border: '1px solid #227bce', marginTop:'10px', overflowX: 'scroll' }}>
                            <div style={{ width: '2000px', marginTop: '-5px' }}>
                                <DataTable ref={dt} value={stock} className="p-datatable-striped" globalFilter={globalFilter} emptyMessage="No Search Result." rowClassName={rowClass}>
                                    ...
                                    <Column field="bdate"  header="Added Date" />
                                    <Column field="item_no"  header="Item No" />
                                    <Column style={{textAlign:'left',width:'200px'}} field="item_name"  header="Item Name" />
                                    <Column style={{textAlign:'right'}} field="gross_weight"  header="Gross Weight" />
                                    <Column style={{textAlign:'right'}} field="less_weight"  header="Less Weight" />
                                    <Column style={{textAlign:'right'}} field="net_weight"  header="Net  Weight" />
                                    <Column style={{textAlign:'right'}} field="quan"  header="Stock (Qty)" />
                                    <Column style={{textAlign:'right'}} field="total_weight"  header="Total Weight" />
                                    <Column style={{textAlign:'right'}} field="price_type"  header="Price Type" />
                                </DataTable>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-grid" style={{ padding: '10px 15px' }}>
                    ...
                    <div className="p-col-1 submitFormButton">
                        <Link to="/stock-list" className="Link">
