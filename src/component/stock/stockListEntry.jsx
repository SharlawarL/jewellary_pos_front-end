...
export default function DataTableFilterDemo(props) {
        ...
        stockService.getStock(data).then((response) =>{
            ...
            if (response['data']['status'] === 1) {
                ...
                response['data']['data'].map((data)=>{
                    data['tot_gross_weight']  = (parseFloat(data['tot_gross_weight'])).toFixed(3)
                    data['tot_less_weight']  = (parseFloat(data['tot_less_weight'])).toFixed(3)
                    data['tot_net_weight']  = (parseFloat(data['tot_net_weight'])).toFixed(3)
                    total_stock       = parseFloat(total_stock)     + parseFloat(data['total_quan'])
                    gross_weight       = parseFloat(gross_weight)     + parseFloat(data['tot_gross_weight'])
                    less_weight       = parseFloat(less_weight)     + parseFloat(data['tot_less_weight'])
                    net_weight       = parseFloat(net_weight)     + parseFloat(data['tot_net_weight'])
                })

                let blank = { entry_no : '.',
                                purity: ''}

                let tdata = { entry_no:'Total',
                                total_quan : total_stock,
                                tot_gross_weight : (parseFloat(gross_weight)).toFixed(3),
                                tot_less_weight : (parseFloat(less_weight)).toFixed(3),
                                tot_net_weight : (parseFloat(net_weight)).toFixed(3),
                            }
    ...
    const rowClass = (data) => {
        return {
            'row-accessories': data.entry_no === 'Total'
        }
    }
            ...
            <div className="continerPonitBox">
                ...
                <div className="datatable-filter-demo">
                    <div className="card" style={{ width: '100%', padding: '10px 10px' }}>
                        <h3 style={{ margin: '0px 0px 15px' }}>Stock Entry List</h3>
                        ...
                        <div style={{ height: '50vh', border: '1px solid #227bce', marginTop:'10px', overflowX: 'scroll' }}>
                            <div style={{ width: '100%', marginTop: '-5px' }}>
                                <DataTable ref={dt} value={stock} className="p-datatable-striped" globalFilter={globalFilter} emptyMessage="No Search Result." rowClassName={rowClass}>
                                    <Column field="entry_no"  header="Entry No" />
                                    <Column field="bdate"  header="Date" />
                                    <Column style={{textAlign:'right'}} field="total_items"  header="Items" />
                                    <Column style={{textAlign:'right'}} field="total_quan"  header="Qty" />
                                    <Column style={{textAlign:'right'}} field="tot_gross_weight"  header="Gross Weight" />
                                    <Column style={{textAlign:'right'}} field="tot_less_weight"  header="Less Weight" />
                                    <Column style={{textAlign:'right'}} field="tot_net_weight"  header="Net Weight" />
                                </DataTable>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-grid" style={{ padding: '10px 15px' }}>
                    ...
                    <div className="p-col-1 submitFormButton">
                        <Link to="/stock-entry-list" className="Link">
